...
**Figure 1. LDT-TargetDB analysis pipeline.** Five input data layers are integrated through a four-step processing pipeline to produce ranked targets with multi-dimensional validation.
...
**Figure 2. Database overview.** (A) TSI distribution across 2,663 surface proteins. (B) Number of pockets per protein. (C) TSI versus LDT scoring by nucleophile type.
...
**Figure 5. Structure quality assessment.** (A) Mean pLDDT versus LDT score. (B) Pocket count distribution.

![Figure 6: Covalent comparison.](results/figures/figure7_covalent_comparison.png)

**Figure 6. Covalent strategy comparison.** Distribution of optimal covalent strategy per target.
...
To evaluate the contribution of each data layer, we benchmarked four model variants against 17 known nuclear medicine targets: (i) Expression-only (TSI rank), (ii) LDT Chemistry-only (LTS rank), (iii) Full model without extracellular filter, and (iv) Full model with extracellular accessibility. Performance was assessed by AUROC and enrichment of known targets in the top 1%, 5%, and 10% of predictions.

![Figure 8: Known targets validation.](results/figures/figure3_validation.png)

**Figure 8. Validation against known targets.** (A) Ten targets mapped onto TSI landscape. (B) DepMap essentiality vs tumor specificity.

![Figure 10: Benchmark curves.](results/figures/figure_benchmark_roc.png)

**Figure 10. Benchmark ROC and PR curves.** Four model variants compared against 17 known targets.

## RESULTS

### Extracellular Surface Target Universe

Of 2,799 surface proteins, 2,663 had expression data, 2,490 had available AlphaFold structures, and 2,473 completed structural analysis. ![Figure 3: Expression profiles.](results/figures/figure4_expression_features.png)

**Figure 3. Tumor expression profiles.** (A) Tumor median log2(TPM) for top 30 targets. (B) Tumor positive rate.

Binding pockets were detected in all 2,473 proteins, with 2,378 (96.2%) harboring LDT-qualifying nucleophilic residues. Extracellular topological domain annotation confirmed extracellular accessibility for 157/200 (78.5%) top targets (Figure 2). The overall analysis framework is illustrated in Figure 1.
