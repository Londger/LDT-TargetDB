...
<sup>\*</sup>To whom correspondence should be addressed. Email: liyiliang@irm-cams.ac.cn; †Email: zhouxinglong@irm-cams.ac.cn
...
LDT-TargetDB (https://ldt-targetdb.streamlit.app) is a computational framework for systematic prioritization of surface protein targets potentially amenable to ligand-directed transfer (LDT) covalent radiopharmaceutical development. The framework integrates surface proteome annotation, pan-cancer expression profiling, extracellular topology validation, 3D structural pocket analysis, and covalent chemistry scoring with empirical pKa prediction. Applied to 2,473 surface proteins, 2,378 (96.2%) harbored LDT-qualifying nucleophilic residues within their largest detected pocket, and post hoc extracellular validation confirmed topology for 157 of 200 top-ranked targets. Benchmarking against 14 analyzable known nuclear medicine targets revealed AUROC = 0.621 for the full model, with expression alone performing best (AUROC = 0.747). Ablation confirmed TSI as the dominant predictor, while LDT chemistry provides orthogonal value. Multi-dimensional validation including normal tissue risk and binder tractability assessment supported translational plausibility. An interactive web interface with downloadable data is freely available without login.

## INTRODUCTION

Radioligand therapy (RLT) has transformed clinical oncology. The approval of [¹⁷⁷Lu]Lu-PSMA-617 (Pluvicto) for metastatic castration-resistant prostate cancer and [¹⁷⁷Lu]Lu-DOTATATE (Lutathera) for neuroendocrine tumors established targeted radionuclide therapy as a pillar of cancer treatment [1,2,33]. The radiopharmaceutical pipeline has expanded rapidly, with over 60 clinical-stage programs targeting more than 25 unique proteins [3]. Despite this momentum, the field confronts a fundamental bottleneck: only a small number of surface targets—primarily PSMA, SSTR2, and FAP—are clinically exploited [4,34]. Expanding the target repertoire is essential for extending RLT to additional cancer types and addressing tumor heterogeneity and therapeutic resistance.
...
Among the 200 top-ranked targets, multi-dimensional validation supported biological plausibility: of the top 30 targets queried in Open Targets, 21 had cancer-associated disease annotations; all of the top 20 targets queried in Ensembl Compara were conserved across >=5 model organisms; TCGA pan-cancer mutation analysis suggested that top-ranked targets were not universally mutated across cancer cohorts (no target showed detectable mutations in all 37 cohorts; cohort-level mutation detection ranged from 0 to 32 of 37 cohorts); and GO enrichment of the top 30 targets identified 20 significantly enriched biological process terms (FDR < 0.05), predominantly related to cell adhesion, transmembrane transport, and epithelial development.
